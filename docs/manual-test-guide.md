...
진행률은 100개 단위로 표시한다. 터미널의 왼쪽 시간은
`[논리시간 01시간 32분 36.41초]`처럼 보이지만, 로그 파일에는 과제 형식에 맞는 누적 초
값인 `[5556.41]`로 저장된다. 터미널 로그 사이의 시간이 크게 뛰는 것은 그 사이 작업별
상세 출력을 생략했기 때문이며, 논리 시계 자체는 모든 처리와 통신 시간을 계속 반영한다.
